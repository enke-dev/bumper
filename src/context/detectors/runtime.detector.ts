...
/** Detect the JS runtime a repo targets (bun wins over node when signalled: a `bun@…`
 * packageManager field, a bun lockfile, or a `.bun-version` pin). */
export async function detectRuntime(cwd: string): Promise<Runtime> {
  const pkg = await readPackageJson(cwd);
  if (pkg?.packageManager?.startsWith('bun')) {
    return Runtime.Bun;
  }
  if (await anyExists(cwd, ['bun.lock', 'bun.lockb', '.bun-version'])) {
    return Runtime.Bun;
  }
  return Runtime.Node;
}
